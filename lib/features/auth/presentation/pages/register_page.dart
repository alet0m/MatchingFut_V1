...
class _RegisterPageState extends ConsumerState<RegisterPage> {
  ...
  Future<void> _handleRegister() async {
    ...
    try {
      ...
      // Si no hay sesión (confirmación de email habilitada), intentar login automático
      if (response.session == null) {
        try {
          final signInResp = await authService.signIn(
            email: _emailController.text.trim(),
            password: _passwordController.text,
            keepSession: true,
          );
          if (signInResp.user == null) {
            throw Exception('Debes confirmar tu correo para continuar');
          }
        } catch (e) {
          // Mostrar guía de verificación
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Revisa tu correo para verificar la cuenta o intenta iniciar sesión.',
                ),
          ...
          return; // No navegar sin sesión
        }
      }

      // Ya autenticado: actualizar tag (pasa RLS) y navegar a onboarding
