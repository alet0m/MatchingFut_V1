...
import '../../../shared/utils/platform_storage.dart';
...
class AuthService {
  ...
  Future<AuthResponse> signUp({
    ...
    // Calcular URL de redirección para verificación de correo (web)
    // Para Flutter Web con GoRouter (hash strategy por defecto), usamos /#/welcome
    final String? emailRedirectTo =
        kIsWeb ? "${Uri.base.origin}/#/welcome" : null;
  ...
  // Reenviar correo de verificación si el usuario no lo recibió
  Future<void> resendVerificationEmail({required String email}) async {
    ...
    try {
      ...
      // Si la API cambia de firma en futuras versiones, dejamos el error visible para ajustar
      rethrow;
    }
  }
  ...
  Future<void> signOut() async {
    try {
      ...
      // 1.1. Limpieza adicional de almacenamiento del navegador (Web)
      // Esto evita que el navegador restaure sesiones previas automáticamente
      try {
        await clearAuthStorage();
        print('✅ Almacenamiento web de autenticación limpiado');
      } catch (e) {
        print('⚠️ No se pudo limpiar almacenamiento web: $e');
      }
