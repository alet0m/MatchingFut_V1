...
-- Primero crear tabla profiles si no existe
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    date_of_birth date,
    position text,
    skill_level text DEFAULT 'principiante',
    bio text,
    is_active boolean DEFAULT true,
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.recruitment_posts (
    ...
    CONSTRAINT recruitment_posts_author_fkey FOREIGN KEY (author_id) REFERENCES public.profiles(id) ON DELETE CASCADE,
