-- Deploy/repair RPC functions used by the app
-- Ensures correct numeric casting for round() and exposes functions to PostgREST

-- get_comuna_team_ranking: returns JSON ranking for a comuna
...
-- get_hottest_sectors: returns JSON with most disputed sectors in comuna
...
-- team elo history (for completeness)
...
-- Permissions: expose RPC to anon/authenticated roles
GRANT EXECUTE ON FUNCTION public.get_comuna_team_ranking(uuid) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_hottest_sectors(uuid, integer) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_team_elo_history(uuid) TO anon, authenticated, service_role;
